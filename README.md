...
- Use relative path for the `urdf_path` in `config/arx5.yaml` based on the dynamic library loading path. Thanks [Jimmy Wu](https://jimmyyhwu.github.io/) for the PR.
...
Robot and controller defaults are loaded from `config/arx5.yaml` when a controller starts. The file is parsed at runtime, so changing joint limits, gripper calibration, motor IDs, gains, or controller timing only requires restarting the process; rebuilding the SDK is not required. A custom file can be selected with `ARX5_CONFIG_FILE` or the three-argument constructor, for example:

```python
controller = arx5.Arx5JointController("X5", "can0", "/path/to/arx5.yaml")
```

The explicit path takes priority over `ARX5_CONFIG_FILE`, followed by the SDK default `config/arx5.yaml`. YAML values are validated against the selected model's degree of freedom and motor ID range before CAN communication starts. The file contains `robots` and `controllers` sections; edit the matching model and controller entries for joint limits, motor mapping, gripper calibration, gravity, gains, or timing. Existing code that passes `RobotConfig` and `ControllerConfig` objects remains supported.

For C++ programs that use the default two-argument constructor, select a file with:

```sh
ARX5_CONFIG_FILE=/path/to/arx5.yaml ./build/test_joint_controller
```

Python code can load the same file explicitly:

```python
robot_config = arx5.load_robot_config("X5", "/path/to/arx5.yaml")
controller_config = arx5.load_controller_config(
    "joint_controller", robot_config.joint_dof, "/path/to/arx5.yaml"
)
controller = arx5.Arx5JointController(robot_config, controller_config, "can0")
```

Changes are applied when the next controller process starts. The SDK does not reload safety limits while a robot is running.
