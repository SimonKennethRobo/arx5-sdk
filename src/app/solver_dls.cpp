...
Arx5Solver::dls_inverse_kinematics(Eigen::Matrix<double, 6, 1> target_pose_6d, Eigen::VectorXd current_joint_pos,
                                   ...
                                   double orientation_tolerance, double max_joint_step)
{
    ...
    for (int iteration = 0; iteration < max_iterations; ++iteration)
    {
        ...
        Eigen::Matrix<double, 6, 6> damped_task_matrix =
            jacobian.data * jacobian.data.transpose() +
            damping_squared * Eigen::Matrix<double, 6, 6>::Identity();
        const Eigen::Matrix<double, 6, 1> task_update = damped_task_matrix.ldlt().solve(error);
        Eigen::VectorXd joint_step = jacobian.data.transpose() * task_update;
